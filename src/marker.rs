//! Map overlay that draws the current GPS position, heading, the track, the
//! connected board (its own GPS) and every remote LoRa node relayed by it, each
//! with its optional path.
...
/// One remote LoRa node to draw: its live position (if known) and its path,
/// both in the node's own palette color so several nodes read apart.
pub struct RemoteDraw {
    /// Live position, or `None` when the node has not been heard this session.
    pub pos: Option<Position>,
    /// The node's path so far, already gated by the page (empty when hidden).
...
pub struct GpsLayer {
    ...
    /// Live position of the connected board's own GPS.
    ...
    /// The remote LoRa nodes relayed by the connected board, one marker and
    /// dashed path each.
    pub remotes: Vec<RemoteDraw>,
    /// The user->target line: the board it goes to and the color to draw it in
    /// (the tracked board when tracking, otherwise the connected board). `None`
    /// when there is no such board with a position.
    pub distance_line: Option<(Position, Color32)>,
    ...
    /// Draw the line to the target dotted rather than solid.
...
impl Plugin for GpsLayer {
    fn run(
        ...
        // A recorded path as a dashed polyline, told apart from the solid phone
        // track. Shared by the connected board's path and every remote's.
        let dash_track = |track: &[Position], color: Color32| {
            if track.len() >= 2 {
                ...
                painter.extend(Shape::dashed_line(
                    &points,
                    Stroke::new(sizes.track, color),
                    8.0,
                    6.0,
                ));
            }
        };
        dash_track(&self.beacon_track, beacon_color);
        for remote in &self.remotes {
            dash_track(&remote.track, remote.color);
        }

        // The line from the current position to the distance target (the
        // tracked board, or the connected board when not tracking), in that
        // board's color. Drawn under the markers. The distance label on it is
        // painted by the map page (see `MyApp::distance_label`), after the
        // rotation pass, so its angle can be set outright.
        if let (Some(pos), Some((target, line_color))) = (self.current, self.distance_line) {
            let screen = projector.project(pos).to_pos2();
            let target_screen = projector.project(target).to_pos2();
            let stroke = Stroke::new(sizes.distance_line, line_color);
            if self.distance_dotted {
                ...
                painter.add(Shape::line_segment([screen, target_screen], stroke));
            }
        }
